...
function wlEnvInit(){
	if($(window).height() < 700){
		console.log("loading JS for android phones");
	    wlCommonInit();
	}else{
		require([ "layers/core-web-layer", "layers/mobile-ui-layer" ], dojoInit);
		console.log("loading JS for android Tablets");
	}
}
				...
				var CartView = Backbone.View.extend({
					...
					render: function(cartDrugs,el){
						...
						_.each(cartDrugs.models,function(cartDrug){
							...
							if(($("#"+targetEl).children(".squares").length+1) % 16 == 0){
								pageNumber++;
						...
						$(".cartNavIcon").remove();
						$(".cartNavCheckout").remove();
						var checkoutButton = "<button class=\"cartNavCheckout mblRoundRect\"data-dojo-type=\"dojox.mobile.Button\">Checkout</button>";
						$(".checkoutPage.rightPane").append(checkoutButton);
						$(".rightPane").css("display","inline-block");
						return this;
						}
						});
				...
				var HistoryRefillView = Backbone.View.extend({
					...
					render: function(cartDrugs,prescDrugs,el){
						...
						_.each(cartDrugs.models,function(cartDrug){
							...
							if(($("#"+targetEl).children(".squaresHistory").length+1) % 8 == 0){
								pageNumber++;
								var addView = new dojox.mobile.SwapView({
									id: el+pageNumber,
									selected: false,
							...
							$("#"+buttonName).on("click",function(){
								...
								if(!dijit.registry.byId(detail_id)){
									console.log("Generating "+detail_id);
									$("#detailsHistoryPage h1").remove();
									var heading1 = new dojox.mobile.Heading({
										label: cartDrug.get("medicationName")
									});
									var detailView = dijit.registry.byId("detailsHistoryPage");
									heading1.placeAt(detailView.containerNode,"first");
									heading1.startup();
...
var CheckoutView = Backbone.View.extend({
	...
	render: function(drugName){
		...
		$(".mblListItemRightIcon").on("click",function(){
							...
							that.remove();
							if($("#checkoutRect").children().length == 0){
								$(".cartText").text("There are no items in your cart");	
							}
							that.render();
						});
	}
